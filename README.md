...
v0.1.9 根据真实素材预览继续收敛 v0.1.8 的封面构图：

- 创建任务时，`poster.jpg` 候选图使用真实 **2:3 竖版卡片**预览，不再沿用 Episode / fanart 的 16:9 横卡。
- 手机端 poster 候选仍可横向滑动，但卡片宽度按竖版海报缩小，避免一张海报占满整屏高度。
- 自动截图内部保留 `standard / soft / strong` 三档融合参数，当前默认使用 **strong**。
- 默认羽化宽度由原先约 2.5% 扩大到约 **14% 的目标短边**，让前景更大范围融入背景。
- 羽化使用弯曲透明度过渡，而不是窄范围线性淡出，减少明显的矩形边界感。
- 模糊背景进一步增强，并适度压暗、降饱和，让完整前景主体更自然地“长”进背景里。
- 仍保持“不裁切原始前景”的原则；原生目标比例素材不会被额外羽化。

## v0.1.8：封面构图优化

v0.1.8 解决竖屏、4:3、超宽屏等视频直接抽帧后，在影视中心卡片中被二次裁切导致主体缺失的问题。

所有从视频自动生成的图片都采用“**固定目标比例 + 原画面完整保留 + 同帧模糊背景补齐**”策略：
...
前景层始终使用等比 `fit`，不会为了填满画布而中心裁切；只有装饰性的模糊背景层允许裁切。手工上传的 poster / fanart 保持用户原有构图，只做 JPG 格式转换，不套用自动填充效果。
...
- poster 自动截图标准化为 2:3；fanart 与 Episode 自动截图标准化为 16:9。
- poster 候选图在创建任务时使用 2:3 竖版预览；fanart / Episode 保持 16:9。
- 自动截图不裁切原始前景画面；比例差异区域使用同帧模糊背景和宽范围羽化边缘补齐。
- 手工上传 artwork 保持用户构图，不应用自动截图的填充与羽化效果。
